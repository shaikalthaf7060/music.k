...
export function findOnlineYouTubeId(title, artist) {
  ...
  // 1. Exact match on title or combined key
  ...
  return "4NRXx6U8ABQ"; // Fallback to verified music stream
}
...
class YouTubePlayerController {
    ...
    try {
      this.player = new window.YT.Player(containerId, {
        ...
        events: {
          ...
          onError: (event) => {
            console.warn('YouTube playback notice, switching to direct stream:', event.data);
  ...
  playTrack(track) {
    // 1. If track has direct audio preview stream (e.g. from live search)
    ...
    // 2. Otherwise play via YouTube IFrame API
    this.isUsingHtmlAudio = false;
    this.audioElement.pause();
    const vId = track.youtubeId || findOnlineYouTubeId(track.title, track.artist);
  ...
  pause() {
    if (this.isUsingHtmlAudio) {
      this.audioElement.pause();
      this.notifyListeners('stateChange', 2);
      return;
    }
  ...
  setVolume(volumeFraction) {
    this.audioElement.volume = Math.max(0, Math.min(1, volumeFraction));
  ...
  getDuration() {
    if (this.isUsingHtmlAudio) {
      return this.audioElement.duration || 210;
    }
